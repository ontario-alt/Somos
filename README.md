...
| Company card (shared execs' AMEX) | Parsed (`etl/parse_amex.py`) from AMEX's own "Transaction Details" charge and credit exports (`AMEX_Debit_to_Date.xlsx` / `AMEX_Credits_to_Date.xlsx`) -- one row per transaction, tagged by the export's own Card Member field so a single combined file covers every exec on the card. Statement "AUTOPAY PAYMENT" lines (no category, pays down the balance) are identified and excluded from all expense totals -- they aren't a business expense or a refund of one. Powers the Executive Expenses page: filter by exec, expense type (from the export's own Category), and a location bucket matched against the merchant's own city/state (real data, captioned as merchant location rather than confirmed travel for online/national merchants). Also surfaces two review lists -- likely shared/group charges (a Restaurant/Entertainment charge over a configurable threshold) and potential personal expenses (a configurable set of personal-leaning categories or an unusually large one-off) -- both explicit heuristics, not determinations, with the specific triggering rule shown next to every flagged row. Tune the categories/thresholds in `config.py` as firm policy is set. |
